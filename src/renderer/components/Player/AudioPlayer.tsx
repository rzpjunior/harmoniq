...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ track, isPlaying, togglePlayPause, handleTrackEnd }) => {
  ...
  return (
    <div className={`rounded-xl fixed bottom-2 flex items-center justify-between p-4 bg-grayHq text-gray-400 ${isSidebarMinimized ? 'w-[calc(100%-4.9rem)]' : 'w-[calc(100%-16.4rem)]'}`}>
        <div className="flex items-center space-x-4">
            <button onClick={togglePlayPause} className=" flex items-center justify-center w-12 h-12 bg-primaryHq text-white rounded-full">
              {isPlaying ? (
                  <FaPause />
                ) : (
                  <FaPlay />
